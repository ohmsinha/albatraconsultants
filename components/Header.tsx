...
import { Menu, X, ChevronDown, Utensils, HeartPulse, Leaf, Globe, Lock, ArrowRight, Server, Clock, Handshake, MessageSquare, ShieldCheck, Search } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <nav className="hidden md:flex items-center space-x-8 h-full">
              ...
              <Link to="/blogs" className={`text-sm font-medium hover:text-primary transition-colors ${isActive('/blogs') ? 'text-primary' : 'text-slate-700'}`}>
                Insights
              </Link>
          ...
          <div className="md:hidden bg-white border-t border-slate-100 max-h-[calc(100vh-5rem)] overflow-y-auto">
            <div className="px-4 pt-2 pb-6 space-y-1">
              ...
              <Link 
                to="/blogs" 
                onClick={() => setIsOpen(false)}
                className={`block px-3 py-2 text-base font-medium rounded-md ${isActive('/blogs') ? 'text-primary bg-primary/5' : 'text-slate-700 hover:text-primary hover:bg-slate-50'}`}
              >
                Insights
              </Link>
              <Link 
